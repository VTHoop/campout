...
8. **A district may publish more than one calendar** — check explicitly for a school-specific variant before assuming a single district-wide calendar is the whole picture; absence of evidence after checking is fine to record as such in TC, but don't skip the check. **A page section existing is not evidence a program is currently operating.** District pages can keep a prior year's school-specific section (e.g. a year-round calendar) long after the program has ended. Don't draft a calendar from a section just because it's on the page; confirm it's live for the target year first. No school-specific or year-round calendar is in scope for 2026-27.
